...
import { lugha, pipe_args, pipe_desugartc, pipe_lp, pipe_read } from "../types";

export class DesugarTCNode extends DesugarTC {
    constructor(
        file: string,
        rd: string,
        wd: string,
    ...
    async visitImport(
        ...
        const cache = Cache.get_instance("desugar_tc");
        let module = cache.has_mod(modPath)
            ? cache.get_mod(modPath)
            : new Module(name, null, `desugar`);
